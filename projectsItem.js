const projects = [
  {
    id: 1,
    title: "Luna Ai",
    imageUrl: "/image.png",
    description: "A web application that allows users to get mental health support.",
    githubLink: "https://github.com/Abhishek-Maurya2/Ai-Bot",
    categories: [{ title: "WebApp" }],
    techstack: ["MongoDB", "Node.js", "React.js"],
  },
  {
    id: 2,
    title: "Chat App",
    imageUrl:
      "/image copy 3.png",
    description: "An AI-powered cross-platforn chat application that provides personalized responses.",
    githubLink: "https://github.com/Abhishek-Maurya2/sera",
    categories: [{ title: "python" }],
    techstack: ["Python", "Flask", "React Native", "SQL"],
  },
  {
    id: 3,
    title: "Dharma Agent",
    imageUrl:
      "/image copy.png",
    description: "An Ai agent capable of performing various tasks.",
    githubLink: "https://github.com/Abhishek-Maurya2/Flask-RAG-chatbot",
    categories: [{ title: "python" }],
    techstack: ["Python", "Flask", "React.js", "SQL"],
  },
  {
    id: 4,
    title: "Rolla Cafeteria",
    imageUrl:
      "/image copy 2.png",
    description: "A cloud kitchen management system that allows users to manage their operations.",
    githubLink: "https://github.com/Abhishek-Maurya2/sweets",
    categories: [{ title: "WebPage" }],
    techstack: ["Firebase", "Node.js", "React.js"],
  },
  {
    id: 5,
    title: "Bolo",
    imageUrl:
      "/image copy 4.png",
    description: "A question and answer platform that allows users to ask and answer questions.",
    githubLink: "https://github.com/Abhishek-Maurya2/Speakx",
    categories: [{ title: "WebApp" }],
    techstack: ["MongoDB", "Node.js", "React.js", "Express"],
  },
];
export default projects;
